...
def test_reserve_sync_resources_leaves_gap_for_exit_task():
    ...
    assert resources.reusable_sync_colors == (0, 1, 2, 3)
    assert resources.dedicated_sync_colors == (4,)
    assert resources.available_local_task_ids == list(range(8, 16))
    assert resources.sync_entrypoints == (17, 18, 19, 20)


def test_reserve_sync_resources_can_opt_into_reuse_pool():
    resources = benchmarking.reserve_sync_resources(
        list(range(21)),
        list(range(8, 21)),
        reuse_sync_colors_for_kernel=True,
    )

    assert resources.available_colors == [0, 1, 2, 3] + list(range(5, 17))
...
def test_sync_rectangle_code_contains_reference_helpers():
    ...
    assert ".configure_sync_routes = true" in code.header
