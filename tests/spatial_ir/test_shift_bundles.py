...
def test_each_source_advances_its_own_switch_once():
    ...
    # A source only has to flip its own router. WSE-2 does that on the last data wavelet:
    # a SWITCH_ADV on the same output queue drops a wavelet once a back-pressured send fills it.
    # WSE-3 keeps SWITCH_ADV; origin-pooled destinations still need a traveling control wavelet.
    if constants.ARCH == 'wse2':
        assert '.advance_switch = true' in sending
        assert 'SWITCH_ADV' not in sending
        assert '.control = true' not in sending
    else:
        assert 'SWITCH_ADV' in sending
        assert '.advance_switch = true' not in sending
