...
class TestCanonicalizeSpatialIR(unittest.TestCase):

    def test_canonicalize_nochange(self):
        kernel = spir.Kernel(
    ...
    def test_canonicalize_singlephase(self):
        kernel = spir.Kernel(
            name='test',
            parameters=[],
            arguments=[],
            body=[
                spir.DataflowBlock([spir.Identifier('i', 0), spir.Identifier('j', 0)],
                                   spir.SubgridExpression(_make_range(0, 20), _make_range(0, 30)), [
                                       spir.RelativeStreamDeclaration(
                                           ...
                                           _make_number(-1), _make_number(1))
                                   ]),
                spir.ComputeBlock([spir.Identifier('i', 0), spir.Identifier('j', 0)],
  ...
  phase {
    dataflow i, j in [0:20 , 0:30] {
      stream<f32> s = relative_stream(-1, 1)
    }
    dataflow i, j in [20:30 , 0:30] {
      stream<f32> s = relative_stream(1, -1)
    }
    compute i, j in [0:30 , 0:30] {

    }
  }
}'''

    def test_canonicalize_multiphase(self):
        kernel = spir.Kernel(
            name='test',
            parameters=[],
            arguments=[],
...
def _make_number(num: int):
    return spir.Expression(spir.ConstantLiteral(num, spir.ScalarType.i16), spir.ScalarType.i16)
