...
| `samples/spatial/sort/` | Sorting networks over `2^L` PEs holding `K` keys each: `batcher_oddeven_1D` (a color per matching), `batcher_oddeven_bundled_1D` (the widest phases bundled onto one color pair), `batcher_oddeven_wse3_1D` (the same, pooling the rest by origin rather than by direction, which reaches `L = 4` on WSE-3), `bitonic_sort_1D` (channel reuse via router switches, WSE-3), and `odd_even_sort_1D_looped` (N neighbour rounds as a runtime loop, four static channels) |
