...
| `samples/spatial/sort/` | Sorting networks over `2^L` PEs holding `K` keys each: `batcher_oddeven_1D` (a color per matching) and `batcher_oddeven_bundled_1D` (the widest phases bundled onto one color pair) |
