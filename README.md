...
```bash
sptlc samples/benchmarks/laplacian_128_128_80.sptl --param I=128 --param J=128
```
...
To compile a GT4Py stencil file to SPADA IR and then to CSL:

```bash
python -m spatialstencil.cli.gt4py_to_spatial samples/stencils.py laplace 128 128 80
```

This produces `.spst` (Stencil IR) and `.sptl` (Spatial IR) files, which can then be passed to `sptlc`.

### Running on Hardware

After compiling with `cslc`, use the Python runtime to execute the kernel:

```python
from spatialstencil.runtime.runtime import run_kernel
import numpy as np

in_field  = np.random.rand(128, 128, 80).astype(np.float32)
out_field = np.zeros_like(in_field)

run_kernel("compiled_output/", in_field=in_field, out_field=out_field)
```

The runtime reads the `metadata.json` generated by `sptlc` to determine the PE grid layout and I/O mapping automatically.
